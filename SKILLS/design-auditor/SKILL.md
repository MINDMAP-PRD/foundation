...
Before starting substantive work for a foundation-based project, read:

1. [`../SKILL_INTERCONNECTION_MAP.md`](../SKILL_INTERCONNECTION_MAP.md)
2. the governed package, if one already exists

This skill is downstream of the governed package workflow. It reviews UI artifacts; it does not create the main project package.

For foundation-based projects with a governed package, assume the default design-system baseline is `shadcn/ui` unless the package explicitly records an approved override. Audit custom component work more strictly when a suitable `shadcn/ui` primitive should have been used instead.
